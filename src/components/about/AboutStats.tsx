  ...
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
    {stats.map((stat, i) => (
      <ScrollReveal key={stat.label} delay={i * 0.12} direction="up">
        ...
        >
          <span className="text-h1 font-bold leading-none text-primary">{stat.value}</span>
          <span className="text-sm mt-3 leading-snug text-text-secondary">{stat.label}</span>
        </SpotlightCard>
      </ScrollReveal>
    ))}
  </div>
